...
export default ({email, isOwner}) => {
    ...
    const ShowOrders = () => {
        ...
        return filteredData.map(order => {
            ...
            const statusColor = status === 'vytvorena' ? 'orange' : status === 'zaplatena' ? 'green' : status === 'odmietnuta' ? 'orangered' : status === 'ocakavana' ? '#D4A121' : status === 'prevodom' ? '#6B3030' : status === 'dobierka' ? '#2c1111' : status === 'odoslana' ? 'navy' : 'black';
            return (
                <tbody key={orderId}>
                        ...
                        <td style={{color: statusColor}}>
                            ...
                            <Form.Control
                                style={{width: '80%', float: 'left', 
                                    backgroundColor: statusObj[_id] === 'vytvorena' ? 'orange' : statusObj[_id] === 'zaplatena' ? 'green' : statusObj[_id] === 'odmietnuta' ? 'orangered' : statusObj[_id] === 'ocakavana' ? '#D4A121' : statusObj[_id] === 'prevodom' ? '#6B3030' : statusObj[_id] === 'dobierka' ? '#2c1111' : statusObj[_id] === 'odoslana' ? 'navy' : statusColor, 
                                    color: "whitesmoke"}}
                                as="select"
